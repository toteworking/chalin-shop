...
// ส่วนของการเพิ่มหรือแก้ไขรายงานการซื้อสินค้า
$message = "";
$action = isset($_GET['action']) ? $_GET['action'] : '';

// ตรวจสอบว่าเป็นคำขอ AJAX เพื่อดึงรายละเอียดการซื้อ
if ($action == 'get_purchase_details' && isset($_GET['purchase_id'])) {
    $purchase_id = intval($_GET['purchase_id']);

    $details_sql = "SELECT pi.Quantity, pi.PriceAtPurchase, p.ProductName
    ...
    while ($row = $details_result->fetch_assoc()) {
        $items[] = [
            ...
            'price' => number_format($row['PriceAtPurchase'], 2)
        ];
    }
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // รับค่าจากฟอร์ม
    $purchase_date = isset($_POST['purchase_date']) ? $_POST['purchase_date'] : date('Y-m-d H:i:s');
    $supplier_id = isset($_POST['supplier_id']) ? intval($_POST['supplier_id']) : 0;
    $shop_id = isset($_POST['shop_id']) ? intval($_POST['shop_id']) : 0; // เพิ่ม ShopID สำหรับการอัปเดตสต็อก
    $products = isset($_POST['products']) ? $_POST['products'] : [];

    if ($supplier_id > 0 && $shop_id > 0 && count($products) > 0) {
        ...
        try {
            ...
            foreach ($products as $item) {
                ...
                $purchase_price = isset($item['purchase_price']) ? floatval($item['purchase_price']) : 0;
            ...
            // เพิ่มรายการซื้อลงในตาราง Purchases
            $purchase_sql = "INSERT INTO Purchases (PurchaseDate, SupplierID, TotalPrice) VALUES (?, ?, ?)";
            $stmt_purchase = $conn->prepare($purchase_sql);
            $stmt_purchase->bind_param("sid", $purchase_date, $supplier_id, $total_price);
            $stmt_purchase->execute();
            $purchase_id = $stmt_purchase->insert_id;
            $stmt_purchase->close();

            // เพิ่มรายการสินค้าที่ซื้อลงในตาราง Purchase_Items
            $purchase_item_sql = "INSERT INTO Purchase_Items (PurchaseID, ProductID, Quantity, PriceAtPurchase) VALUES (?, ?, ?, ?)";
            $stmt_item = $conn->prepare($purchase_item_sql);
            foreach ($items_to_insert as $item) {
                $stmt_item->bind_param("iiid", $purchase_id, $item['product_id'], $item['quantity'], $item['purchase_price']);
                $stmt_item->execute();
            }
            $stmt_item->close();

            $conn->commit();
            $message = "<div class='message success'>เพิ่มใบรายงานการซื้อสำเร็จ!</div>";
...
// ดึงรายการซื้อทั้งหมดเพื่อแสดงในตาราง
$purchases_sql = "SELECT p.PurchaseID, p.PurchaseDate, p.TotalPrice, s.SupplierName
                  FROM Purchases p
                  JOIN Suppliers s ON p.SupplierID = s.SupplierID
                  ORDER BY p.PurchaseDate DESC";
$purchases_result = $conn->query($purchases_sql);

// ดึงรายการซัพพลายเออร์และสินค้าสำหรับ dropdowns
$suppliers_sql = "SELECT SupplierID, SupplierName FROM Suppliers ORDER BY SupplierName ASC";
                        ...
                        <div class="col-lg-6">
                            ...
                            <div class="card shadow mb-4">
                                ...
                                <div class="card-body">
                                    ...
                                    <form action="manage_purchases_reports.php" method="POST">
                                        ...
                                        <div class="form-group">
                                            <label for="shop_id">หน้าร้านที่รับสินค้าเข้า:</label>
                                        ...
                                        <button type="button" id="add-product-btn">เพิ่มสินค้า</button>

                                        <button type="submit">บันทึกรายการซื้อ</button>
                                    </form>
                                </div>
                            </div>
                            ...
                            <div class="card shadow mb-4">
                                ...
                                <div class="card-body">
                                    ...
                                    if ($purchases_result->num_rows > 0) {
                                        echo "<table>";
                                        echo "<thead><tr><th>รหัสบิล</th><th>วันที่ซื้อ</th><th>ซัพพลายเออร์</th><th>ราคารวม</th><th>Action</th></tr></thead>";
                                        echo "<tbody>";
                                        while ($row = $purchases_result->fetch_assoc()) {
                                            ...
                                            echo "<td><button class='detail-btn' onclick='showDetails(" . $row["PurchaseID"] . ")'>ดูรายละเอียด</button></td>";
                                            echo "</tr>";
                                        }
                                        echo "</tbody>";
                                        echo "</table>";
                                    } else {
                                        echo "<p>ไม่พบข้อมูลการซื้อ</p>";
                                    }

                                    $conn->close();
                                    ?>
                                    <!-- Details section to be populated by JS -->
                                    <div id="purchase-details" class="details-section">
                                        <h3>รายละเอียดใบรายงานการซื้อ: <span id="purchase-id-display"></span></h3>
